...
function _hcTenSubscribe(room) {
  ...
  _hcTenChannel = sbL.channel('cleaning-tenant-rt')
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'lounge_data' }, async payload => {
      if (payload.new?.type === 'hc_done') loadHouseCleaning(room);
    })
...
async function loadHouseCleaning(room) {
  ...
  /* ── Rotation timeline ── */
  _renderHcRotation(cycleStart, cyclePos, hcDoneMap, absRows, room, rot);
...
/* ── ROTATION TIMELINE ──────────────────────────────────── */
function _renderHcRotation(cycleStart, cyclePos, hcDoneMap, absRows, myRoom, rot) {
  ...
  rotEl.innerHTML = '<div class="rot-tl">' + rot.map((r, i) => {
    ...
    /* "you" label on the tenant's own room rows */
    const youTag = (myRoom && r === myRoom)
      ? `<span style="font-size:9px;font-weight:600;color:var(--cc-gold);margin-left:4px;letter-spacing:0.06em;">you</span>`
      : '';
    ...
    return `<div class="${rowClass}">
      ...
      <div class="rot-tl-body">
        <div class="rot-tl-info">
          <p class="rot-tl-room">${r}${youTag}</p>
